...
Measured against API spec **0.126.1**. The SDK wraps 40 of the 41 REST operations in
...
| Account | `getAccount()`, `getAccountUsage()` | Account methods are currently available on `QTSurfer`; see [account.md](account.md). |
...
| Live execution | `startLive`, `getLive`, `stopLive`, `listPublicLive`, `updateLive`, `updateLiveParams`, `getLiveSignals` | The generated `mintLiveConnectionToken` is deliberately not exposed until Java has a managed WebSocket abstraction. |
